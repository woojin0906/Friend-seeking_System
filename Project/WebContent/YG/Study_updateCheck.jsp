			...
			if( !title.equals("") && !time.equals("") && !person.equals("") && !gender.equals("") && !type.equals("") && !arrival.equals("") && !context.equals("")) {
				try {
					...
					if(!number.equals("") && !nickname.equals("")) {
						stmt = conn.prepareStatement("update study set nickname = '" + nickname + "', title = '" + title +  "', promisetime = '" + time + "', count = '" + person + "', sex = '" + gender + "', category = '" + type + "', dest = '" + arrival + "', main = '" + context + "', writetime = '" + date + "'where number ='" + number + "'");
					}
					
					// 게시판에서 글 작성 후 글보기로 넘어온 경우 넘버가 없으므로 닉이 공백이 아님
					else if(!nickname.equals("")){
						stmt = conn.prepareStatement("update study set nickname = '" + nickname + "', title = '" + title +  "', promisetime = '" + time + "', count = '" + person + "', sex = '" + gender + "', category = '" + type + "', dest = '" + arrival + "', main = '" + context + "', writetime = '" + date + "'where nickname ='" + nickname + "'order by number desc limit 1");
						
					}
					
					stmt.executeUpdate();
					
					response.sendRedirect("Study_writePost.jsp");
