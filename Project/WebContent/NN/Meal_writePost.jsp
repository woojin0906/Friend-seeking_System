	...
	// 글 작성 후 바로 글 보기로 넘어갈 때 넘버 값을 모르기 때문에 사용자의 NICK을 사용
		...
		while(rs.next()) {
			%>
             ...
             <tr>
                 <th>음주여부</th>
                 <td id="hidden" colspan="3"><input type="hidden" name="_alcohol" value="<%=rs.getString("drink") %>"/><%=rs.getString("drink") %></td>        
             </tr>
             <tr>
                 <th>도착지</th>
         ...
         <input id="btnPost" type="button" value="목록" onclick="location.href='NN_friend.jsp?_index=1'">
         </form>
             	<%		 	}
     ...
     <%
        try {
	        ...
	        if(!number.equals("")) {
				...
				rs = stmt.executeQuery("select * from mealParticipate where name = '" + nick + "'order by number desc limit 1");
			}
